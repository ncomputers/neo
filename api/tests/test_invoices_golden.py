...
def test_rounding_adjustment_line():
    items = [{"price": 100.33}]
    bill, invoice = build_invoice_json(items, "unreg")
    assert bill["rounding_adjustment"] == -0.33
    assert invoice["rounding_adjustment"] == -0.33
    assert bill["total"] == round(bill["subtotal"] + bill["rounding_adjustment"], 2)
