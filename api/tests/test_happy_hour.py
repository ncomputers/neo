...
def test_discount_windows_apply():
    windows = [
    ...
    b1 = billing_service.compute_bill(
        [item], "unreg", happy_hour=windows, now=time(10, 30)
    )
    b2 = billing_service.compute_bill(
        [item], "unreg", happy_hour=windows, now=time(11, 30)
    )
    b3 = billing_service.compute_bill(
        [item], "unreg", happy_hour=windows, now=time(12, 30)
    )
...
def test_discount_boundary():
    ...
    at_start = billing_service.compute_bill(
        [item], "unreg", happy_hour=windows, now=time(10, 0)
    )
    at_end = billing_service.compute_bill(
        [item], "unreg", happy_hour=windows, now=time(11, 0)
    )
...
def test_invoice_context_includes_discount():
    windows = [{"start": "10:00", "end": "11:00", "percent": 10}]
    settings = get_settings()
    settings.happy_hour = windows
    ...
    invoice = billing_service.build_invoice_context(
        items, "unreg", happy_hour=windows, now=time(10, 30)
    )
